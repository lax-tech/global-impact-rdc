...
export default function User() {
  ...
  useEffect(() => {
    axiosInstance
    .get(`output_money/${serviceId}/`)
    .then((res) => {
      console.log('success output_money')
      ...
      setUSERLIST(res.data.map((el, index) => ({
        ...
        name: el.number,
        company: el.operator,
        isVerified: el.is_done,
        date: new Date(el.date_add).toLocaleDateString(),
        status: 'banned',
        role: el.amount
      }))
      );
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers
                    ...
                    .map((row) => {
                      const { id, name, role, status, date, company, avatarUrl, isVerified } = row;
